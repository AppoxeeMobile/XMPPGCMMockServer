...
public class GCMMessageHandler extends DefaultMessageHandler {
  ...
  protected Stanza executeMessageLogic(MessageStanza stanza, ServerRuntimeContext serverRuntimeContext,
    ...
    try {
      ...
      if (jsonObject.get(JSON_MESSAGE_ID).toString().startsWith("dr2")) {
        return null ;   // added by Evyatar: do not reply to ACK messages that the client sent in reply to delivery receipt
      }
      ...
      // reply with SERVICE_UNAVAILABLE if msg sent containd _SU,
      // but for 1 in every 5 minutes, this functionality will be disabled
      if (jsonObject.get(JSON_MESSAGE_ID).toString().endsWith("~~~SU") && (GregorianCalendar.getInstance().get(Calendar.MINUTE) % 5 != 0)) {
        Stanza outboundStanza = createNackServiceUnavailableMessageStanza(stanza, jsonObject);
        MessageRelayJobImpl messageRelayJob = new MessageRelayJobImpl(ThreadLocalRandom.current().nextLong(ACK_DELAY_MS), from, outboundStanza, serverRuntimeContext);
        serviceContext.getMessageRelayManager().addJob(messageRelayJob);
        return null;
      }
  ...
  private static Stanza createAckMessageStanza(Stanza original, JSONObject jsonObject) throws EntityFormatException {
    ...
    if (true) return buildMessageStanza(original, payload);
    // no from & to
  ...
  private static Stanza buildMessageStanza(Stanza original, String payload) {
    StanzaBuilder builder = new StanzaBuilder("message");
    if (original.getAttributeValue("id") != null) {
      builder.addAttribute("id", original.getAttributeValue("id"));
    }
    GCMMessage.Builder gcmMessageBuilder = new GCMMessage.Builder();
    gcmMessageBuilder.addText(payload);
    builder.addPreparedElement(gcmMessageBuilder.build());
  ...
  /**
   * Create SERVICE_UNAVAILABLE Nack Message Stanza
   *
   <message>
   <gcm xmlns="google:mobile:data">
   {
   ...
   "error_description":"The server could not process the request in time"
   }
   </gcm>
   </message>
   ...
   * @throws EntityFormatException
   */
  private static Stanza createNackServiceUnavailableMessageStanza(Stanza original, JSONObject jsonObject) throws EntityFormatException {

    Map<String, Object> message = new HashMap<>();
    message.put(JSON_MESSAGE_TYPE, JSON_NACK);
    message.put(JSON_FROM, jsonObject.get(JSON_TO));
    message.put(JSON_MESSAGE_ID, jsonObject.get(JSON_MESSAGE_ID));
    message.put(JSON_ERROR, "SERVICE_UNAVAILABLE");
    message.put(JSON_ERROR_DESCRIPTION, "The server could not process the request in time");

    String payload = JSONValue.toJSONString(message);

    return buildMessageStanza(original, payload);
  }
